...
if __name__ == '__main__':
    ...
    lluvia = generar_lluvia(det_size,det_delta,det_height,seed=0,n=int(1e6))

    total_muons = 0
    measured_angles = np.array([])
    for ang in [0,np.pi/8,np.pi/4,3*np.pi/8,np.pi/2,5*np.pi/8,3*np.pi/4,7*np.pi/8]:
        measured_angles_i,total_muons_i = sim_rotados(lluvia,det_size,det_height,det_xy,det_delta,freq,ang0=ang,seed=0)
    ...
    np.savetxt(f'data_plano_varios.csv',data,delimiter=',')