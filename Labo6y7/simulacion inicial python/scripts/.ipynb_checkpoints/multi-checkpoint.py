...
#    z_o = rng.uniform(z0-z_length/2,z0+z_length/2,size=N) # se repite el mismo plano a lo largo de varias alturas de generación, se samplea desde una uniforme para obtener ese valor
    z_o = np.full(N,100) # la altura de generación es el punto medio entre los centelladores
...
if __name__ == '__main__':
    ...
    margin = 0 # det_delta+2*det_height # distancia a la que se amplia a cada lado el plano de generación
    z_margin = 0 #(det_delta+2*det_height)/2 # distancia la que se amplia hacia arriba y a bajo el volumen de generación
    freq = 1 # vueltas por segundo
    n_chunks = 4 # cuántos procesos en paralelo quiero correr
    
    lluvia = generar_lluvia('n',det_size,det_delta,det_height,margin,z_margin,seed=0,n=int(1e8))
    for ang in [0,np.pi/8,np.pi/4,3*np.pi/8,np.pi/2,5*np.pi/8,3*np.pi/4,7*np.pi/8]
    	measured_angles,total_muons = sim_rotados(lluvia,det_size,det_height,det_xy,det_delta,freq,ang0=ang,seed=0)
    	...
    	np.savetxt(f'angulos/data_2_rot_{np.degrees(ang)}.csv',data,delimiter=',')
