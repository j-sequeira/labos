...
def generar_lluvia(mode,det_size,det_delta,det_height,margin,seed=0,rate=1/60,t_f=0,n=0,multiplier=1.2):
...
if __name__ == '__main__':
    ...
    margin = 0 # distancia a la que se amplia a cada lado el plano de generación
    ...
    for i in range(len(angs)):
        lluvia = generar_lluvia('n',det_size,det_delta,det_height,margin,seed=i,n=int(1e5))
        measured_angles,total_muons = sim_rotados(lluvia,det_size,det_height,det_xy,det_delta,freq,ang0=angs[i],seed=0)
        data[0] += total_muons
        data.extend(measured_angles)
    
    data = np.array(data) # guardo la cantidad total de muones generados y los ángulos en los que se detectaron en un csv
    np.savetxt('data_prueba_fijo.csv',data,delimiter=',')