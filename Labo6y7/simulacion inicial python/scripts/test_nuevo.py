...
def batch_generate_muon_t(det_size,det_delta,det_height,r0,rate,rng,N):
    area = 4*np.pi*r0**2
    ...
    theta_o = rng.uniform(0,np.pi/2,size=N)
    phi_o = rng.uniform(0,2*np.pi,size=N)
#    x_o = r0*np.sin(theta_o)*np.cos(phi_o)
    x_o = np.zeros(N)
    y_o = r0*np.sin(theta_o) #*np.sin(phi_o)
    z_o = det_height+det_delta/2+r0*np.cos(theta_o)
...
def generar_lluvia(det_size,det_delta,det_height,r0,seed=0,rate=1/60,t_f=0,n=0,multiplier=1.2):
    rng = np.random.default_rng(seed) # defino el generador de números aleatorios

    muon_times,muon_starts,muon_flechas = batch_generate_muon_t(det_size,det_delta,det_height,r0,rate,rng,n)
...
if __name__ == '__main__':
    ...
    r0 = (det_delta/2+det_height)*2
    freq = 1 # vueltas por segundo
    
    lluvia = generar_lluvia(det_size,det_delta,det_height,r0,seed=0,n=int(1e7))

    total_muons = 0
    measured_angles = np.array([])
    for ang in [0,np.pi/8,np.pi/4,3*np.pi/8,np.pi/2,5*np.pi/8,3*np.pi/4,7*np.pi/8]:
        measured_angles_i,total_muons_i = sim_rotados(lluvia,det_size,det_height,det_xy,det_delta,freq,ang0=None,seed=0)
        measured_angles = np.concatenate([measured_angles,measured_angles_i])
        total_muons += total_muons_i
    ...
    np.savetxt(f'data_2_rot_test.csv',data,delimiter=',')
